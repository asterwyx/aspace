    ...
    , m_updateTime()
{
    ...
    connect(
        m_weatherDBusInterface.data(), &WeatherInterface::futureWeatherUpdated, [&](const QList<FutureWeather> &futureWeather) {
            this->m_updateTime = QDateTime::currentDateTime();
...
{
    QDBusPendingReply<QList<FutureWeather>> futureWeather =
        m_weatherDBusInterface->getFutureWeather(m_locationModel->getLocationId());
